alter table public.organizations
  add column can_issue_certificates boolean not null default false;

alter table public.certificates
  add column issuer_organization_id uuid references public.organizations(id);

update public.certificates certificate
set issuer_organization_id = profile.organization_id
from public.workers worker
join public.profiles profile on profile.id = worker.id
where worker.id = certificate.worker_id;

alter table public.certificates
  alter column issuer_organization_id set not null;

drop function public.persist_evaluated_attempt(
